...
void unestef(const char * c, int pos)
{
	...
	while (!in.eof())
	{
		...
		if(a == args[0])
		{
			...
			if(std::filesystem::exists(workingDir + "/" + commitPath))
			{
				elog("savarsire exista deja");
				return;
			}
